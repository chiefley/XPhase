...
def main() -> None:
    ...
    print("Offset center-frequency feedline sweep only.")
    print("This is not bandwidth evaluation, current-ratio optimization, or NEC pattern verification.")
    print("Sign convention: positive offset adds physical coax to port 2, the forward +Y reference source.")
    ...
    print("sweep: common length 75 ft to 95 ft by 5 ft; offset -25 ft to +25 ft by 5 ft")
    print("length convention: port1_length = common, port2_length = common + offset")
    print("")
    ...
    print(
        "rank  common  offset  p1_len  p2_len  Z1box(ohms)          Z2box(ohms)          "
    ...
    for rank, result in enumerate(displayed_results, start=1):
        ...
        print(
            f"{rank:>4}  "
            f"{c.common_length:>6.1f}  "
            f"{c.offset:>6.1f}  "
            f"{c.port1_length:>6.1f}  "
            f"{c.port2_length:>6.1f}  "
            f"{_format_impedance(c.port1_box_impedance_ohms):>19}  "
            f"{_format_impedance(c.port2_box_impedance_ohms):>19}  "
...
def _parse_args(argv=None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run an offset center-frequency feedline sweep."
    )
    parser.add_argument(
        "--limit",
        type=_positive_int,
